...
%             % Making some changes to the code on 05/03/2020
%             % It now gives a phase locking value for each channel pair and
%             % each trial across time. Previously it used to give a plv
%             % value for each time point, each channel pair across trials. 
%             plv(:, channelCount, compareChannelCount, conditionCount) = abs(sum(exp(1i*(channelData(:, dataSelectArr(:, conditionCount)) - compareChannelData(:, dataSelectArr(:, conditionCount)))), 1))/size(eegData, 2);
        end
    end
end
plv = squeeze(plv);
