function scatterHistDiff1(x, y, xeb, yeb, colors)
...
% Modified by Sanchit Gupta, Cognition Lab, 
% Indian Institute of Science, Bangalore on 27th September, 2022
...
minAx   = min([x,y],[], 'all'); %min([xlims ylims]); 
maxAx   = max([x,y],[], 'all'); %max([xlims ylims]);
rangeAx = range([minAx maxAx]);
minAx   = minAx - rangeAx*0.05; 
maxAx   = maxAx + rangeAx*0.05;
...
% % show bar in inset
% histogram(h_inset, x-y,'DisplayStyle', 'stairs', 'edgecolor', [0 0 0]);
% 
% try
% %     % do statistics on the pairs, paired t-test
% %     [~, pval] = signrank(x, y);%ttest(x, y);
% %     mysigstar(h_inset, nanmean(x-y), max(get(h_inset, 'ylim')*1.1), pval);
% 
%     % Put a triangle indicating the median
%     plot(h_inset, nanmedian(x-y), max(get(h_inset, 'ylim')*1.1), 'v', 'Color', [0 0 0]);
%     l = xline(h_inset,0);
%     set(l, 'color', 'k', 'linestyle', '--', 'linewidth', 0.5);
% 
% end
