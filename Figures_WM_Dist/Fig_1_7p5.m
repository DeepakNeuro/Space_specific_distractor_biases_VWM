...
doPermutationTest(m_do.*[0 1],'smaller','left');
...
% Output of fitting the von Mises to the pooled response errors
load('../../Data/Behavior/MLE_vonMisesFit_kappa_pooledErrorSaved.mat');

means = [delta_k_ds_do_cu_mu, delta_k_ds_do_uc_mu];
errors = [delta_k_ds_do_cu_std, delta_k_ds_do_uc_std];

labels = {'DS-DO_{CU}', 'DS-DO_{UC}'};
...
b = barh(means, 'EdgeColor', 'none');
b.FaceColor = 'flat';
b.BaseValue = 0.4; 
b.BarWidth = 0.3; 
% Add horizontal error bars
for i = 1:numel(means)
    errorbar(means(i), i, errors(i), 'horizontal', ...
        'k', 'LineStyle', 'none', 'LineWidth', 1.2);
end

% Axes settings
set(gca, 'YTick', 1:2, 'YTickLabel', labels, 'YDir', 'reverse');
xlabel('\Delta k value');
ylabel('Condition');

box off;
grid on;

% Make bars start visually from 0.4 and extend left
xlim([-1.2 0.4]);   % 0.4 on right, negatives to the left
set(gca, 'XDir', 'reverse'); % flips direction
