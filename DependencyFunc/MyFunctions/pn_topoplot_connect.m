function [] = pn_topoplot_connect(displayStruct, loc_file)
...
%         cmapPos = round((displayStruct.connectStrength - 0)/(1 - 0)*(size(cM, 1) - 1) + 1); % Change made by Sanchit on 05/03/2020
    end
end
for kk = 1:numChanPairs
%     if sqrt(diff(y(displayStruct.chanPairs(kk, :)))^2 +  diff(x(displayStruct.chanPairs(kk, :)))^2) >= 0.3
