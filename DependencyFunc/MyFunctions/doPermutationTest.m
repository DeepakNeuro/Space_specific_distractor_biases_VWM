function [p, stats, pVal_permutationTest] = doPermutationTest(temp, sided, tailed)
...
p = signrank(temp(:,1), temp(:,2),'tail',tailed); %stats.t.p; 
...
%% Bayes Factor
pVal_permutationTest = permutationTest(temp(:,1), temp(:,2), 1e4,'sidedness', sided,'plotresult',0);

addpath('/media/hdd/Sanchit/Exogenous_Project/Toolboxes/bayesFactor/'); 
[bf10, bf_ttest_p] = bf.ttest(temp(:,1), temp(:,2),'tail',tailed);
if display_flag
