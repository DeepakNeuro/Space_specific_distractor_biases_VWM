classdef Violin < handle
    ...
    methods (Static)
        function [density, value, width] = calcKernelDensity(data, bandwidth, width)
            ...
            idx_min = min([prctile(value, 10), min(data)]);
            idx_max = max([prctile(value, 90), max(data)]);
            density = density(value >= idx_min & value <= idx_max);
            value = value(value >= idx_min & value <= idx_max);
%             value(1) = min(data);
%             value(end) = max(data);
