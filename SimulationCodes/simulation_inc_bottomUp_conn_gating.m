%{
Increased the bottom up connection strength to simulate distractor gating
effects.
%}

function [ang_cu_mat, ang_uc_mat, metrics] = simulation_inc_bottomUp_conn_gating(ori_deg, dist_params, rng_seed)
...
stimulus_cu = stim*v_cu';                          stimulus_uc = stim*v_uc';                              stimulus_dist = 5*stim*v_d'; 
...
for i=1:nsteps
  ...
  if (i>stimoff)
      ...
      if (i>time_cueON) % After cue onset
          ...
          % Bottom-up inputs from VC to PFC
          IE_cu1=IE_cu1 + (1/300)*GEE1*rE_cu;  IE_uc1=IE_uc1 + (1/300)*GEE1*rE_uc; % Bottom-up connection from VC to PFC
