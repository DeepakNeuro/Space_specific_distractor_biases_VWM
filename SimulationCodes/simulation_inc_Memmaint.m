%{
Increased top down feedback to simulation the stronger memorandum
maintenence. 

%}

function [ang_cu_mat, ang_uc_mat, metrics] = simulation_inc_Memmaint(ori_deg, dist_params, rng_seed)
...
for i=1:nsteps,
  ...
  if (i>stimoff)
      ...
      if (i>time_cueON) % After cue onset
          IE_cu = IE_cu + 1.2*2*GEE2*rE_cu1;  IE_uc = IE_uc + 2*GEE2*rE_uc1;
